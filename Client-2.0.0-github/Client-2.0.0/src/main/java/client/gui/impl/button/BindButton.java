    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if(ClickGui.getInstance().gui.getCurrentState() == ClickGui.Gui.OLD) {
            ...
            if (this.isListening) {
                ...
                Client.textManager.drawStringWithShadow(this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getCurrentState().toString(), this.x + 2.3f, this.y - 1.7f - (float) ClientGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
            }
        }
    }
