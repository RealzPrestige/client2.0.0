...
import static client.util.EntityUtil.calculateDamage;
...
public class ObiAssist extends Module {
    ...
    public void onUpdate() {
        ...
        if (AutoCrystal.getInstance().isEnabled() && target != null) {
            if (slot != -1) {
                if (delayTimer.passedMs(delay.getCurrentState().longValue())) {
                    if (mc.player.inventory.currentItem != slot) {
                        if (packet.getCurrentState()) {
        ...
        try {
            ...
            if (!(target.getDistance(mc.player) > range.getCurrentState())) {

                float range = AutoCrystal.getInstance().placeRange.getCurrentState();

                float targetDMG = 0.0f;
                float minDmg = AutoCrystal.getInstance().minDamage.getCurrentState();
                ...
                if (targetPos != null && render.getCurrentState()) {
                ...
                if (packet.getCurrentState()) {
                    if (slot != -1) {
                        ...
                        if (packethand.getCurrentState() && hand != null) {
