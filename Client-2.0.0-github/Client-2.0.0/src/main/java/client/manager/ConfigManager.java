...
public class ConfigManager implements Util {
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        switch (setting.getType()) {
            ...
            case "Enum":
                try {
                    EnumConverter converter = new EnumConverter(((Enum) setting.getCurrentState()).getClass());
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("client").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("client/" + name + "/"))) {
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("client").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("client/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("client/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("client", ""));
                writer.close();
            } else {
    ...
    public JsonObject writeSettings(Feature feature) {
        ...
        for (Setting setting : feature.getSettings()) {
            if (setting.isEnumSetting()) {
                EnumConverter converter = new EnumConverter(((Enum) setting.getCurrentState()).getClass());
                object.add(setting.getName(), converter.doForward((Enum) setting.getCurrentState()));
                continue;
            }
            if (setting.isStringSetting()) {
                String str = (String) setting.getCurrentState();
                setting.setValue(str.replace(" ", "_"));
            }
            try {
